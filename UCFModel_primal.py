...
# logtime_data ={}
# function(args, log_time=logtime_data)
def timeit(method):
    def timed(*args, **kw):
        ts = time.time()
...
class TemporalRelationGraph(torch.nn.Module):
    ...
    def forward(self, graph_out, init_input, is_test):
        ...
        for seq_ind in range(1,self.seq_size+1):     
            ...
            for scaleID in range(len(new_scales)):
                ...
                #print('scale',new_scales[scaleID])
                #print(self.relations_scales[seq_ind-1][scale_index][idx_relations_randomsample[0]])
    ...
    def select_idx(self,relations_scales,scale,num_frames):
        def is_Arithmetic_progression(arr,distance):
            # return if the arr is an Arithmetic progression
            #print(arr)
            arr_dis= np.diff(arr)
