...
only using seq/ratio when calculate accuracy 
...
def test(epoch,dataloader, model, criterion1,criterion2,  opt,writer,log_file, is_test=True):
    ...
    with torch.no_grad():
        ...
        for i, (edge_features, node_features, adj_mat, node_labels, relation_label, graph_label, sequence_ids, subject_ids) in enumerate(
                dataloader, 0):
        '''
        for i, (edge_features, node_features, adj_mat, graph_label,tem_features) in enumerate(dataloader):
            ...
            ##pre_label = pre_label.view(batch, opt.ratio_num, -1)
            ##gt_labels = gt_labels.view(batch, opt.seq_size, -1)
            ##gt_labels = gt_labels[:,:opt.ratio_num,:]
            ...
            for ratio in range(opt.ratio_split_num):
                seq_i = int (ratio*opt.seq_size/opt.ratio_split_num)+1
                correct[0, ratio] = (pre_label[:, seq_i, 0] == gt_labels[:, seq_i, 0]).sum()
                test_correct[0, ratio] = correct[0, ratio] + test_correct[0, ratio]
