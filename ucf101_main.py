...
Using train_val_test split to conduct experiments
...
parser.add_argument('--ratio_split_num', type=int, default=10, help='sequence length of rnn')
...
parser.add_argument('--max_num_node', type=int, default=5, help='#node of the spatial graph')
parser.add_argument('--node_feat_dim', type=int, default=2048, help='node state size')  #1024 #512 
parser.add_argument('--edge_feat_dim', type=int, default=2052, help='edge state size')  #1028  #516
...
parser.add_argument('--embedding_dim', type=int, default=1024, help='dim of annotation')
parser.add_argument('--num_bottleneck', type=int, default=512, help='dim of temporal reasoning module')
parser.add_argument('--num_frames', type=int, default=8, help='number of sampled frames in each segment ')
parser.add_argument('--subsample_num', type=int, default=10, help='number of sampled frames in each segment ')
parser.add_argument('--n_steps', type=int, default=3, help='propogation steps number of GGNN')
parser.add_argument('--K_clusters', type=int, default=3, help='number of clusters in K-means')
...
#parser.add_argument('--resume', default='/home/wrq/IJCV/results/ucf_apsplit/3dresnext101_embed1024/ckp',help='path to latest checkpoint')
#parser.add_argument('--resume', default='/home/wrq/IJCV/results/ucf_apsplit/3dresnext101_embed1024/ckp/3619',help='path to latest checkpoint')
parser.add_argument('--resume', default='/home/wrq/IJCV/results/ucf_apsplit/3dresnext101_embed1024/ckp/4601',help='path to latest checkpoint')

parser.add_argument('--logroot', default='/home/wrq/IJCV/results/ucf_apsplit/3dresnext101_embed1024/log',help='path to latest log')
...
parser.add_argument('--l1', type=int, default=8, help='weight of classification loss')
parser.add_argument('--l2', type=int, default=1, help='weight of semantic loss')
parser.add_argument('--featdir', type=str, help='feat dir')
parser.add_argument('--ggnn', action='store_true', help='ggnn model')
parser.add_argument('--l2norm', action='store_true', help='enables cuda')
...
def main(opt):
    ...
    with open(log_file_name,'a+') as file:
        ...
        file.write('embedding dim is %d \n' % opt.embedding_dim)
        file.write('state_dim is %d \n' % opt.state_dim)
        file.write('num_bottleneck is %d \n' % opt.num_bottleneck)
        file.write('logroot is ' + opt.logroot + '\n')
        file.write('K_clusters is %d \n' % opt.K_clusters)
        file.write('subsampling num is %d \n' % opt.subsample_num)
    paths = config.Paths()

    train_datalist = '/home/wrq/IJCV/data/ucf101_trainlist_ap.txt'
    val_datalist = '/home/wrq/IJCV/data/ucf101_vallist_ap.txt'
    test_datalist = '/home/wrq/IJCV/data/ucf101_testlist_ap.txt'
    #train_dataset = dataset(train_datalist, paths.detect_root_ucf_mmdet, paths.img_root_ucf,paths.rgb_resnext_ucf,paths.rgb_resnext_ucf, opt)
    train_dataset = dataset(train_datalist, paths.ap_3dresnext_ucf, opt)
    train_dataloader = DataLoader(train_dataset, batch_size=opt.batch_size, shuffle=True, num_workers=opt.workers, drop_last=False)
    #val_dataset = dataset(val_datalist, paths.detect_root_ucf_mmdet, paths.img_root_ucf, paths.rgb_resnext_ucf,paths.rgb_resnext_ucf,opt)
    val_dataset = dataset(val_datalist, paths.ap_3dresnext_ucf,opt)
    val_dataloader = DataLoader(val_dataset, batch_size=opt.batch_size, shuffle=False, num_workers=opt.workers, drop_last=False)
    #test_dataset = dataset(test_datalist, paths.detect_root_ucf_mmdet, paths.img_root_ucf, paths.rgb_resnext_ucf,paths.rgb_resnext_ucf, opt)
    test_dataset = dataset(test_datalist, paths.ap_3dresnext_ucf, opt)
    ...
    if opt.eval:
        ...
        exit()    
    #print ("Start to test once to build a baseline")
    #val_acc, output = test(1,val_dataloader, model, criterion1, criterion2, opt, writer, log_file_name, is_test=True)
    #tmp_val_acc = np.mean(val_acc)
    #if tmp_val_acc > best_acc:
    #    best_acc = tmp_val_acc

    print ("Start to train and validation.....")
